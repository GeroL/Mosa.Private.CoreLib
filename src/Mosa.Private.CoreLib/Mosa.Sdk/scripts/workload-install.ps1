...
$ManifestBaseName = "Mosa.Sdk.Manifest"

$LatestVersionMap = @{
    "$ManifestBaseName-8.0.100-rc.1" = "1.0.0";
}
...
function Install-MosaWorkload([string]$DotnetVersion)
{
    ...
    $MosaManifestDir = Join-Path -Path $ManifestDir -ChildPath "Mosa.sdk"
...
if (Get-Command $DotnetCommand -ErrorAction SilentlyContinue)
{
    ...
    {
        $InstalledDotnetSdks = Invoke-Expression "& '$DotnetCommand' --list-sdks | Select-String -Pattern '^8|^8'" | ForEach-Object {$_ -replace (" \[.*","")}
    }
    else
    {
...
{
    Write-Host "`n.NET SDK version 8 or later is required to install Mosa Workload."
}
else
{
